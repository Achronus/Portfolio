...
class DetectEnglishText():
  ...
  def __init__(self):
    ...
    self.all_letters = self.letters + self.letters.lower() + ' \t\n' # Including spaces
  
  def load_dictionary(self):
    """
    Opens the dictionary text file and stores all of the words within it into an 
    English words dictonary. Returns the words dictionary.
    """
    # Open file
  ...
  def remove_non_letters(self, message):
    """
    Utility function for the get_english_count function, used to 
  ...
  def is_english(self, message, word_percent=20, letter_percent=85):
    """
    Checks if a message contains English words. Requires a minimum of 20% of the words 
    in the dictionary being being valid and 85% of all characters in the message 
    must be letters or spaces (not punctuation or numbers).\n
